import { ReactiveList, SelectableList } from '../reactive.list';
import { ListItem, DATA } from './_data_/list-item.data';

jest.useFakeTimers();

describe('ReactiveList<ListItem>', () => {
  const findList = (vm: SelectableList<ListItem>) => vm.list;
  const findSelected = (vm: SelectableList<ListItem>) => vm.selected;
  const findListLength = (vm: SelectableList<ListItem>) => findList(vm).length;
  const findSelectedLength = (vm: SelectableList<ListItem>) => findSelected(vm).length;
  const findStatusValue = (vm: SelectableList<ListItem>) => vm.status.value;
