...
import { ReactiveStore, Entity } from './reactive.store';
...
export class PaginatedStore<T extends StoreState, K extends Entity> extends ReactiveStore<T, K> {
   ...
   */
  override reset() {
    const initState = () => ({ ...this.initState(), isLoading: false, isReady: false });
   ...
   */
  override selectItem(id: string, clearAll = true): void {
    if (!id) return;
   ...
   * @param id
   */
  selectPageWithItem(id: string): void {
    if (!id) return;
  ...
  selectPage(page: number, clearSelections = true): boolean {
